...
export function assignComponent(element) {
    ...
    const updateInputComponents = () => {
        [...container.querySelectorAll('.app-cmp-input')].forEach((component, index, inputComponents) => {
            [...component.querySelectorAll('.app-elem-title-no')].forEach((titleNo) => {
                if (titleNo instanceof HTMLElement) {
                    titleNo.textContent = `${index + 1}`;
                }
            });
            [...component.querySelectorAll('.app-cmd-remove-input')].forEach((cmdRemoveInput) => {
    ...
    const calculateResult = () => {
        ...
        [...element.querySelectorAll('output.app-elem-result')].forEach((output) => {
